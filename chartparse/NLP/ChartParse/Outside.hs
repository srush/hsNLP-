...
unOptL (inspan, isemi) (outspan, osemi) = do
  ...
  guard $ (state $ leftEnd inspanLinked) == (state $ leftEnd outspan)  
  return $ (inspan,
            linkedp `times` osemi)
...
unOptR (inspan, isemi) (outspan, osemi) = do
  ...
  guard $ (state $ rightEnd inspanLinked) == (state $ rightEnd outspan)  
  return $ (inspan,
            linkedp `times` osemi)
...
unCombineL (inspan1,_) (inspan2, semi2) (outspan, osemi) = do 
    guard $ not $ simple outspan &&  wm
    (incombineSpan, pcomb) <- combine' inspan1 inspan2
    guard $ incombineSpan == outspan  
    return $ 
...
unprocessCell sentence (i, j) inChart outChart = seeds
    ...
    where 
      ...
      seeds = if (i,j) == (1, n + 1) then 
                 ...
                 [uncomb unCombineL i j k | k <- [j+1..n+1]] ++ 
