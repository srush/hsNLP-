...
readAndCount file1 file2 = do
  ...
  let sents =  map (parseSentence file1. unlines) $ separate "" $ lines contents
  print "ending parsing" 
  let newsents = ngroup (sents::[WordInfoSent]) 100 
  
  counts <- mapM countSome $ zip newsents [0..]
  print "count sets done"
  return $! mconcat counts
      where 
        countSome (ls,n) =  do 
          putStrLn $ "set" ++ (show n)
          return $! mconcat $ map (directCounts2 . toTAGDependency initSemiCounts) ls
